...
cd lzo-${LZO_VERSION}
./configure --quiet --cache-file=$(get_config_cache arm-linux) --build=x86_64-pc-linux-gnu --host=arm-linux --prefix=$PREFIX --disable-shared --enable-static
...
cd xz-${XZ_VERSION}
./configure --quiet --cache-file=$(get_config_cache arm-linux) --build=x86_64-pc-linux-gnu --host=arm-linux --with-pic --disable-shared --prefix=$PREFIX
make -sj$NCPU install
cd ..
